...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "get_analytics") {
    if (!analyticsDataClient) {
      return {
        content: [{ type: "text", text: "Error: Google Analytics client is not initialized." }],
        isError: true,
      };
    }
    const propertyId = process.env.VITE_GA_PROPERTY_ID;
    try {
      ...
      return {
        content: [{ type: "text", text: JSON.stringify({ newUsersLast7Days: newUsers, topPages: topPages }, null, 2) }],
      };
    } catch (error) {
      return {
  ...
  if (request.params.name === "send_notification") {
    if (initError) {
      return {
        content: [{ type: "text", text: `Firebase Init Error: ${initError.message}` }],
        isError: true,
      };
    }
    ...
    if (!title || !body) {
      return {
        content: [{ type: "text", text: "Error: Missing title or body" }],
        isError: true,
      };
    }
    ...
    try {
      ...
      if (tokensSnapshot.empty) {
         return {
          content: [{ type: "text", text: "No subscribed users found." }],
        };
      }
      ...
      if (tokens.length === 0) {
        return {
          content: [{ type: "text", text: "No valid tokens found." }],
        };
      }
      ...
      const message = {
        ...
        webpush: {
          notification: {
            ...
            click_action: 'https://plexreq.netlify.app'
          }
        },
        tokens: tokens
      };
      ...
      return {
        content: [{ type: "text", text: JSON.stringify({ success: true, sent: response.successCount, failed: response.failureCount }, null, 2) }],
      };
    } catch (error) {
      return {
        content: [{ type: "text", text: `Error sending notification: ${error.message}` }],
        isError: true,
      };
    }
  }
